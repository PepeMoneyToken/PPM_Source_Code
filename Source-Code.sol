...
contract Pepemoney is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PepemoneyDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Pepemoney", "Ppm") {
        marketingWallet = payable(0x5102572a97de191f6D2F9b9F1045C4C859a70630);
        devWallet = payable(0x5102572a97de191f6D2F9b9F1045C4C859a70630);
        ...
        dividendTracker = new PepemoneyDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 30000000; // 3%
        swapTokensAtAmount = totalTokenSupply / 30000000; // 3%;
...
contract PepemoneyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Pepemoney public PepemoneyContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PepemoneyContract = Pepemoney(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Pepemoney_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Pepemoney_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Pepemoney contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PepemoneyContract);

            uint256 prevBalance = PepemoneyContract.balanceOf(address(this));
            ...
            {
                uint256 received = PepemoneyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PepemoneyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
